...
import java.text.*;
	...
	public class SVGTransformMatrix{
		 ...
		 */
		public SVGTransformMatrix(SVGTransformMatrix matrix){
			
			transform=new AffineTransform(matrix.getA(), matrix.getB(), matrix.getC(), matrix.getD(), matrix.getE(), matrix.getF());
		}
		...
		public String toString(){
			...
			for(i=0;i<6;i++){
				...
				display=display.concat(matrix[i]+" "); //$NON-NLS-1$
			}	
		...
		public String getMatrixRepresentation(){
			...
			double[] matrix=new double[6];
			transform.getMatrix(matrix);
			String rep="matrix("; //$NON-NLS-1$
			
			rep=rep.concat(" "+format.format(matrix[0])+","); //$NON-NLS-1$ //$NON-NLS-2$
			rep=rep.concat(" "+format.format(matrix[1])+",");			 //$NON-NLS-1$ //$NON-NLS-2$
			rep=rep.concat(" "+format.format(matrix[2])+","); //$NON-NLS-1$ //$NON-NLS-2$
			rep=rep.concat(" "+format.format(matrix[3])+",");			 //$NON-NLS-1$ //$NON-NLS-2$
			rep=rep.concat(" "+format.format(matrix[4])+",");		 //$NON-NLS-1$ //$NON-NLS-2$
			rep=rep.concat(" "+format.format(matrix[5]));			 //$NON-NLS-1$
			rep=rep.concat(")"); //$NON-NLS-1$
