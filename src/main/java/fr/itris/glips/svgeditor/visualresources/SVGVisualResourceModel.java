...
public class SVGVisualResourceModel{
     ...
     */
    private String tabName=""; //$NON-NLS-1$
    
    /**
     * the default id
     */
    private String defaultId=""; //$NON-NLS-1$
    ...
    public SVGVisualResourceModel(SVGVisualResources visualResources, Element resourceElement){
        ...
        defs=visualResources.getDefs(frame);
        
        if(resourceElement!=null && frame!=null){
            
            //the name of the type of the resource
            name=visualResources.getNormalizedString(resourceElement.getAttribute("name")); //$NON-NLS-1$
            
            //gets the name of the tab
            if(visualResources.getBundle()!=null){
                
                defaultId=resourceElement.getAttribute("vresource_newresource"); //$NON-NLS-1$
                tabName=resourceElement.getAttribute("name"); //$NON-NLS-1$
    ...
    protected SVGVisualResourceObject createVisualResourceObject(Element element){
        ...
        if(element!=null){
            ...
            if(childModel!=null){
                ...
                }else if(defs!=null && ! cloneElement.hasAttributeNS(SVGToolkit.xmlnsXLinkNS, "href")){ //$NON-NLS-1$
