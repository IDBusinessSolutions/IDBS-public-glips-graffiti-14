...
public class SVGVisualResourcePatternSizeChooserAttributeWidget extends SVGVisualResourceAttributeWidget{
    ...
    protected void buildComponent(){
        ...
        if(resourceObjectAttribute!=null){
            ...
            final String patternUnitsOther="userSpaceOnUse"; //$NON-NLS-1$
            ...
            for(Iterator it=resourceObjectAttribute.getGroupAttributes().iterator();it.hasNext();){
                ...
                if(att!=null){
                    ...
                    if(nm.equals("x")){ //$NON-NLS-1$
                        
                        xAtt=att;
                        
                    }else if(nm.equals("y")){ //$NON-NLS-1$
                        
                        yAtt=att;
                        
                    }else if(nm.equals("width")){ //$NON-NLS-1$
                        
                        wAtt=att;
                        
                    }else if(nm.equals("height")){ //$NON-NLS-1$
                        
                        hAtt=att;
                        
                    }else if(nm.equals("patternUnits")){ //$NON-NLS-1$
            ...
            final SVGVisualResourceObjectAttribute 	fxAtt=xAtt, fyAtt=yAtt, fwAtt=wAtt, fhAtt=hAtt, 
																			fpatternUnitsAtt=patternUnitsAtt, fatt=null;
            ...
            if(patternChValues!=null && xAtt!=null && yAtt!=null && wAtt!=null && hAtt!=null){
                ...
                String patternUnitsLabel=""; //$NON-NLS-1$
                ...
                JLabel comboLabel=new JLabel(patternUnitsLabel.concat(" :")); //$NON-NLS-1$
                ...
                xTxt.setFont(theFont);
                yTxt.setFont(theFont);
                wTxt.setFont(theFont);
                hTxt.setFont(theFont);
                ...
                if(bundle!=null){
                    ...
                    try{
                        strX=bundle.getString("vresource_".concat(strX)); //$NON-NLS-1$
                        strY=bundle.getString("vresource_".concat(strY)); //$NON-NLS-1$
                        strW=bundle.getString("vresource_".concat(strW)); //$NON-NLS-1$
                        strH=bundle.getString("vresource_".concat(strH)); //$NON-NLS-1$
                    }catch (Exception ex){}
                }

                //the labels corresponding to the entries
                final JLabel 	xLbl=new JLabel(strX.concat("=")),  //$NON-NLS-1$
                					yLbl=new JLabel(strY.concat("=")), //$NON-NLS-1$
                					wLbl=new JLabel(strW.concat("=")),  //$NON-NLS-1$
                					hLbl=new JLabel(strH.concat("=")); //$NON-NLS-1$
                
                //sets the properties for the label
                xLbl.setFont(theFont);
                yLbl.setFont(theFont);
                wLbl.setFont(theFont);
                hLbl.setFont(theFont);
                ...
                //sets the properties for the label
                xUnitLbl.setFont(theFont);
                yUnitLbl.setFont(theFont);
                wUnitLbl.setFont(theFont);
                hUnitLbl.setFont(theFont);
                    ...
                    public void caretUpdate(CaretEvent evt) {
                        ...
                        if(evt.getSource().equals(xTxt)){
                            ...
                            if(fpatternChValues.isDefaultPatternUnits()){
                                ...
                                val=svgEditor.getSVGToolkit().getDoubleValue(xTxt.getText().concat("%"), fpatternChValues.isDefaultPatternUnits()); //$NON-NLS-1$
                                fpatternChValues.getDefBounds().x=val;
                                fxAtt.setValue(format.format(val)+"%");                          //$NON-NLS-1$
                            ...
                            if(fpatternChValues.isDefaultPatternUnits()){
                                ...
                                val=svgEditor.getSVGToolkit().getDoubleValue(yTxt.getText().concat("%"), fpatternChValues.isDefaultPatternUnits()); //$NON-NLS-1$
                                fpatternChValues.getDefBounds().y=val;
                                fyAtt.setValue(format.format(val)+"%");                          //$NON-NLS-1$
                            ...
                            if(fpatternChValues.isDefaultPatternUnits()){
                                ...
                                val=svgEditor.getSVGToolkit().getDoubleValue(wTxt.getText().concat("%"), fpatternChValues.isDefaultPatternUnits()); //$NON-NLS-1$
                                fpatternChValues.getDefBounds().width=val;
                                fwAtt.setValue(format.format(val)+"%");                          //$NON-NLS-1$
                            ...
                            if(fpatternChValues.isDefaultPatternUnits()){
                                ...
                                val=svgEditor.getSVGToolkit().getDoubleValue(hTxt.getText().concat("%"), fpatternChValues.isDefaultPatternUnits()); //$NON-NLS-1$
                                fpatternChValues.getDefBounds().height=val;
                                fhAtt.setValue(format.format(val)+"%"); //$NON-NLS-1$
                    ...
                    public void actionPerformed(ActionEvent evt) {
                        ...
                        if(val!=null && ! val.equals("")){ //$NON-NLS-1$
                            ...
                            if(fpatternChValues.isDefaultPatternUnits()){
                                ...
                                xStr=xStr.concat("%"); //$NON-NLS-1$
                                yStr=yStr.concat("%"); //$NON-NLS-1$
                                wStr=wStr.concat("%"); //$NON-NLS-1$
                                hStr=hStr.concat("%"); //$NON-NLS-1$
                            }

                            bounds.x=svgEditor.getSVGToolkit().getDoubleValue(xStr, fpatternChValues.isDefaultPatternUnits());
                            bounds.y=svgEditor.getSVGToolkit().getDoubleValue(yStr, fpatternChValues.isDefaultPatternUnits());
                            bounds.width=svgEditor.getSVGToolkit().getDoubleValue(wStr, fpatternChValues.isDefaultPatternUnits());
                            bounds.height=svgEditor.getSVGToolkit().getDoubleValue(hStr, fpatternChValues.isDefaultPatternUnits());

                            if(fpatternChValues.isDefaultPatternUnits()){
                                
                                fpatternChValues.setDefBounds(bounds);

                            }else{
                                
                                fpatternChValues.setOtherBounds(bounds);
                            }
