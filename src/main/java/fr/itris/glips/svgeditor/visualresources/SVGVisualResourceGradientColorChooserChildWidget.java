...
public class SVGVisualResourceGradientColorChooserChildWidget extends SVGVisualResourceChildWidget{
    ...
    protected void buildComponent(){
        ...
        if(resourceObject!=null && resourceObject.getChildren().getFirst()!=null){
            ...
            final SVGEditor svgEditor=resourceObject.getResourceModel().getVisualResources().getSVGEditor();
            ...
            String 	stopProperties="", stopColor="", stopOpacity="", offset="", labelAddStop="", labelRemoveStop=""; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$

            if(bundle!=null){
                
                try{
                    widgetLabel=bundle.getString(widgetLabel);
                    stopProperties=bundle.getString("vresource_stopproperties"); //$NON-NLS-1$
                    stopColor=bundle.getString("vresource_stop-color"); //$NON-NLS-1$
                    stopOpacity=bundle.getString("vresource_stop-opacity"); //$NON-NLS-1$
                    offset=bundle.getString("vresource_offset"); //$NON-NLS-1$
                    labelAddStop=bundle.getString("vresource_addstop"); //$NON-NLS-1$
                    labelRemoveStop=bundle.getString("vresource_removestop"); //$NON-NLS-1$
                }catch (Exception ex){}
            }
            ...
            final JLabel 	stopColorLbl=new JLabel(stopColor.concat(" :")), //$NON-NLS-1$
            					stopOpacityLbl=new JLabel(stopOpacity.concat(" :")), //$NON-NLS-1$
            					offsetLbl=new JLabel(offset.concat(" :")); //$NON-NLS-1$
            
            stopColorLbl.setFont(theFont);
            stopOpacityLbl.setFont(theFont);
            offsetLbl.setFont(theFont);
            
            //the panel containing the add and remove button
            JPanel buttonPanel=new JPanel();
            
            //the icons
            final ImageIcon 	addIcon=svgEditor.getResource().getIcon("New", false), //$NON-NLS-1$
            							deleteIcon=svgEditor.getResource().getIcon("Delete", false), //$NON-NLS-1$
            							deleteDisabledIcon=svgEditor.getResource().getIcon("Delete", true); //$NON-NLS-1$

            //the add and remove buttons
            final JButton addBt=new JButton(addIcon), deleteBt=new JButton(deleteIcon);

            //dealing with the buttons' properties
            addBt.setFont(smallFont);
            deleteBt.setFont(smallFont);
            ...
            final JTextField positionValueTxt=new JTextField("", 4); //$NON-NLS-1$
            positionValueTxt.setFont(theFont);
            ...
            final JLabel displayedValue=new JLabel(format.format(val)+" %"); //$NON-NLS-1$
            displayedValue.setPreferredSize(new Dimension(30, 20));
            slider.setPreferredSize(new Dimension(130, 25));
            displayedValue.setFont(theFont);
                ...
                public void setCurrentItem(SVGGradientChooserItem item){
                    ...
                    if(item!=null){
                        ...
                        if(item.getColor()!=null){
                            ...
                            positionValueTxt.setText(item.getXPositionPercent()+"%"); //$NON-NLS-1$
                            displayedValue.setText((int)(item.getOpacity()*100)+"%"); //$NON-NLS-1$
                        ...
                        colorBt.setEnabled(false);
                        colorPanel.setBackground(disabledColorPanelColor);
                        displayedValue.setText("0%"); //$NON-NLS-1$
                        slider.setEnabled(false);
                        positionValueTxt.setText(""); //$NON-NLS-1$
                    }
                    ...
                    if(currentItem!=null){
                        ...
                        if(e.getClickCount()==2){
                            ...
                            if(color!=null && currentItem!=null){
                                ...
                                svgEditor.getSVGColorManager().setCurrentColor(color);
                ...
                public void mouseDragged(MouseEvent e) {
                    ...
                    if(currentItem!=null){
                        ...
                        if(point!=null && canDrag){
                            ...
                            positionValueTxt.setText(currentItem.getXPositionPercent()+"%"); //$NON-NLS-1$
                            refresh();
                        }
                    }
                }
            ...
            final ChangeListener sliderChangeListener=new ChangeListener(){
                ...
                public void stateChanged(ChangeEvent evt) {
                    ...
                    arrowsListener.getCurrentItem().setOpacity(((double)slider.getValue())/100);
                    displayedValue.setText(slider.getValue()+" %"); //$NON-NLS-1$
                    gradientPanel.repaint();
                }
            };
            ...
            disposer=new Runnable(){
                ...
                public void run() {
                    ...
                    final SVGFrame frame=svgEditor.getFrameManager().getCurrentFrame();
                    
                    //sorts the child nodes according to the "offset" attribute value
                    if(frame!=null && fresourceObject!=null && arrowsListener.isModified()){
    ...
    protected class SVGGradientChooserItem{
                ...
                if(attributes!=null){
                    ...
                    for(int i=0;i<attributes.size();i++){
                        ...
                        if(att!=null){
                            ...
                            if(att.getModel().getName().equals("offset")){ //$NON-NLS-1$
                                
                                posResAttribute=att;
                                
                            }else if(att.getModel().getName().equals("style")){ //$NON-NLS-1$
        ...
        protected void applyXPosition(){
            ...
            if(posResAttribute!=null && arrowsPanelSize!=null){
                ...
                posResAttribute.setValue(format.format((int)percent)+"%"); //$NON-NLS-1$
            }
        }
        ...
        protected Color computeColor(){
            ...
            if(styleResAttribute!=null){
                ...
                val=val.replaceAll("\\s*[;]\\s*", ";"); //$NON-NLS-1$ //$NON-NLS-2$
                val=val.replaceAll("\\s*[:]\\s*", ":"); //$NON-NLS-1$ //$NON-NLS-2$
                
                String prop="stop-color"; //$NON-NLS-1$
                ...
                if(ind!=-1){
                    ...
                    val=val.substring(val.indexOf(":")+1, val.indexOf(";")); //$NON-NLS-1$ //$NON-NLS-2$
                    
                    SVGEditor editor=resChild.getChildModel().getVisualResources().getSVGEditor();
                    SVGFrame frame=editor.getFrameManager().getCurrentFrame();
                    
                    color=editor.getColorChooser().getColor(frame, val);
                }
            }
        ...
        protected double computeOpacity(){
            ...
            if(styleResAttribute!=null){
                ...
                String prop="stop-opacity"; //$NON-NLS-1$
                int ind=val.indexOf(prop);
        ...
        protected void applyColor(){
            ...
            if(color!=null && styleResAttribute!=null){
                ...
                if(ind!=-1){
                    ...
                    int ind2=val2.indexOf(";"); //$NON-NLS-1$
                    ...
                    if(ind2!=-1){
                        ...
                        if(val.length()>0 && ! val.endsWith(";")){ //$NON-NLS-1$
                ...
                val=val.concat(prop.concat(":".concat(SVGEditor.getColorChooser().getColorString(color)).concat(";"))); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        protected void applyOpacity(){
            ...
            if(styleResAttribute!=null){
                ...
                if(ind!=-1){
                    ...
                    String val1="", val2=""; //$NON-NLS-1$ //$NON-NLS-2$
                ...
                //creating the new value of the style attribute
                val=val.concat(prop.concat(":".concat(format.format(opacity)).concat(";"))); //$NON-NLS-1$ //$NON-NLS-2$
