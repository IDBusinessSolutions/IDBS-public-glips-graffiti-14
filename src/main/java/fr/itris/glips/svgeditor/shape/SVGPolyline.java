...
/**
 * @author Jordi SUC
 * the class that allows to add, select,modify the properties, delete, transform a polyline on the canvas
 */
public class SVGPolyline extends SVGShape{
	...
	/**
	 * the action listener used to draw the polyline
	 */
	 ...
	 */
	public SVGPolyline(SVGEditor editor) {
	    
		super(editor);
		
		ids.put("id","polyline"); //$NON-NLS-1$ //$NON-NLS-2$
		ids.put("idmenuitem","Polyline"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if(bundle!=null){
			...
			try{
				labels.put("label", bundle.getString("shapepolylinelabel")); //$NON-NLS-1$ //$NON-NLS-2$
				labels.put("undoredocreate", bundle.getString("shapepolylineundoredocreate")); //$NON-NLS-1$ //$NON-NLS-2$
				labels.put("undoredotranslate", bundle.getString("shapeundoredotranslate")); //$NON-NLS-1$ //$NON-NLS-2$
				labels.put("undoredoresize", bundle.getString("shapepolylineundoredoresize")); //$NON-NLS-1$ //$NON-NLS-2$
				labels.put("undoredomodifypoint", bundle.getString("shapepolylineundoredomodifypoint")); //$NON-NLS-1$ //$NON-NLS-2$
				labels.put("undoredorotate", bundle.getString("shapepolylineundoredorotate")); //$NON-NLS-1$ //$NON-NLS-2$
				labels.put("helpcreate", bundle.getString("shapepolylinehelpcreate")); //$NON-NLS-1$ //$NON-NLS-2$
			}catch (Exception ex){}
		}
		...
		menuitem.addActionListener(polylineAction);
		toolItem.addActionListener(polylineAction);
	 ...
	 */
	protected void drawPolyline(SVGFrame frame, Point2D.Double[] points){
		...
		if(frame!=null && points!=null && points.length>1){
			...
			if(getSVGEditor().getSVGSelection()!=null && doc!=null){
				...
				if(parent!=null){
					...
					final Element polyline=doc.createElementNS(doc.getDocumentElement().getNamespaceURI(),"polyline"); //$NON-NLS-1$

					polyline.setAttributeNS(null,"points",value); //$NON-NLS-1$
					String colorString=getSVGEditor().getColorChooser().getColorString(getSVGEditor().getSVGColorManager().getCurrentColor());
					polyline.setAttributeNS(null, "style", "stroke:".concat(colorString.concat(";fill:none;"))); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					...
					if(getSVGEditor().getUndoRedo()!=null){
						...
						if(selection!=null){
							...
							selection.handleNodeSelection(frame, polyline);
	...
	/**
	 * draws what the polyline will be if the user double clicks
	 * @param frame the current SVGFrame
	...
	protected void drawGhost(SVGFrame frame, Graphics2D g, Point2D.Double[] points){
		...
		if(frame!=null && points!=null && points.length>1 && g!=null){
			...
			//draws the new awt polyline to be displayed		
	...
	/**
	 * draws the selection for the polyline
	 * @param frame the current SVGFrame
			...
			if(pts!=null && !pts.equals("")){ //$NON-NLS-1$
				...
				if(points!=null && points.length>0){
					...
					if(ctxt!=null){
						...
						GraphicsNode gnode=ctxt.getGraphicsNode((Element)node);
	...
	public void modifyPoint(SVGFrame frame, SVGSelectionSquare square, Point2D.Double point1, Point2D.Double point2){
		...
		if(frame!=null && square!=null && square.getNode()!=null && square.getNode() instanceof Element && point2!=null){
			...
			try{paintListener=(CanvasPaintListener)modifyPointFrameTable.get(frame);}catch (Exception ex){paintListener=null;}
	...
	/**
	 * used to remove the listener added to draw a polyline when the user clicks on the menu item
	 */
	public void cancelActions(){
	    
		if(polylineAction!=null) {
			
			toolItem.removeActionListener(polylineAction);
			toolItem.setSelected(false);
			toolItem.addActionListener(polylineAction);
			
			polylineAction.cancelActions();
		}
	}
	 ...
	 * 
	 * @author Jordi SUC
	 * the class allowing to get the position and size of the future drawn polyline 
	 */
	protected class PolylineActionListener implements ActionListener{
		...
		protected PolylineActionListener(){
			...
			createCursor=getSVGEditor().getCursors().getCursor("polyline"); //$NON-NLS-1$
		}
		...
		protected void reset(){
			...
			if(isActive){
				...
				PolylineMouseListener pml=null;
				...
				for(it=frames.iterator(); it.hasNext();){
					...
					if(frm!=null && ! mouseAdapterFrames.containsKey(frm)){
						...
						pml=new PolylineMouseListener(frm);
					...
					if(frm!=null){
						...
						try{
							...
							if(mouseListener!=null && ((PolylineMouseListener)mouseListener).paintListener!=null){
								...
								//removes the paint listener
								frm.getScrollPane().getSVGCanvas().removePaintListener(((PolylineMouseListener)mouseListener).paintListener, true);
							}
						}catch (Exception ex){}
		...
		protected class PolylineMouseListener extends MouseAdapter implements MouseMotionListener{
			 ...
			 * @param frame a frame
			 */
			public PolylineMouseListener(SVGFrame frame){
					...
					public void paintToBeDone(Graphics g) {
						...
						if(points.size()>1){
							...
							if(g!=null && scPts!=null){
							    ...
							    svgPolyline.drawGhost(fframe, (Graphics2D)g, scPts);
							}
						}
					}
				};
						...
						if(points.size()>1){
							...
							Runnable runnable=new Runnable(){
								...
								public void run(){
									...
									svgPolyline.drawPolyline(frame, pointsTab);
								}
							};
