...
public class SVGPropertiesSliderWidget extends SVGPropertiesWidget{
    ...
    /** added to fix the mutation issue of events */
	...
	protected void buildComponent(){
		...
		final JTextField textField=new JTextField(val+"", 5); //$NON-NLS-1$
		textField.setHorizontalAlignment(SwingConstants.RIGHT);
		textField.setFont(theFont);
		final JLabel displayedValue=new JLabel("%"); //$NON-NLS-1$
		...
		final MouseAdapter sliderListener=new MouseAdapter(){
			...
			public void mouseReleased(MouseEvent evt) {
				...
				textField.setText(slider.getValue()+""); //$NON-NLS-1$
			}
		};
		...
		final CaretListener textFieldListener=new CaretListener(){
			...
			public void caretUpdate(CaretEvent evt) {
				...
				if(! Double.isNaN(newVal)){
					...
					if(newVal>100){
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								...
								textField.setText(fnewVal+""); //$NON-NLS-1$
					...
					propertyItem.changePropertyValue(newValue);
                    /** set this to true, else the slider will try to set the text value again causing mutation */
		...
		final ChangeListener sliderChangeListener=new ChangeListener(){
			...
			public void stateChanged(ChangeEvent evt) {
                /** if the event is raised because of text field update, ignore and return */
